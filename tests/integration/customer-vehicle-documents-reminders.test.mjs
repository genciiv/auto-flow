...
test("upload kufizon formatin, madhësinë dhe ruan në storage privat me path jo të parashikueshëm", async () => {
  ...
  assert.match(service, /customerVehicle:\s*\{\s*profileId\s*\}/);
});

test("download verifikon user-in pronar dhe përdor signed URL të shkurtër", async () => {
  const route = await source(
    "src/app/api/customer/vehicles/[vehicleId]/documents/[documentId]/download/route.js",
  );

  assert.match(route, /globalRole\s*!==\s*"CUSTOMER"/);
  assert.match(route, /profile:\s*\{\s*userId:\s*session\.user\.id\s*,?\s*\}/);
  assert.match(route, /createSignedUrl\(document\.storagePath,\s*60/);
});

test("cron-i krijon vetëm një bell notification kur dokumenti hyn në dritaren e reminder-it", async () => {
  const service = await source(
    "src/services/customer-vehicle-reminder-notification-service.js",
  );
