...
async function getExecutiveDashboardData(businessId) {
  ...
  const months = Array.from(
    { length: 6 },
    (_, index) => {
      const period = getAppMonthRange(
        now,
        index - 5,
      );

      return {
        key: `${period.year}-${String(
          period.month,
        ).padStart(2, "0")}`,
        label:
  ...
  for (const expense of expenses) {
    const bucket = monthMap.get(
      getAppMonthKey(expense.expenseDate),
    );
  ...
  for (const purchase of purchases) {
    const bucket = monthMap.get(
      getAppMonthKey(purchase.updatedAt),
    );
  ...
  const currentRevenue = payments
    .filter(
      (item) =>
        item.paidAt >= currentStart &&
        item.paidAt < nextStart,
    )
    .reduce(
      (sum, item) =>
  ...
  const currentExpenses = expenses
    .filter(
      (item) =>
        item.expenseDate >= currentStart &&
        item.expenseDate < nextStart,
    )
    .reduce(
      (sum, item) =>
        sum + Number(item.amount || 0),
  ...
  const previousExpenses = expenses
    .filter(
      (item) =>
        item.expenseDate >= previousStart &&
        item.expenseDate < currentStart,
    )
    .reduce(
      (sum, item) =>
  ...
  const currentPurchases = purchases
    .filter(
      (item) =>
        item.updatedAt >= currentStart &&
        item.updatedAt < nextStart,
    )
    .reduce(
      (sum, item) =>
  ...
  const currentIssuedInvoices =
    invoices.filter(
      (invoice) =>
        invoice.status !== "DRAFT" &&
        invoice.createdAt >= currentStart &&
        invoice.createdAt < nextStart,
    );

  const previousIssuedInvoices =
    invoices.filter(
      (invoice) =>
        invoice.status !== "DRAFT" &&
        invoice.createdAt >= previousStart &&
        invoice.createdAt < currentStart,
    );

  const currentNetRevenue =
    sumInvoiceNetRevenue(
      currentIssuedInvoices,
    );

  const previousNetRevenue =
    sumInvoiceNetRevenue(
      previousIssuedInvoices,
    );

  const currentCogs =
    sumInvoiceCogs(currentIssuedInvoices);

  const previousCogs =
    sumInvoiceCogs(previousIssuedInvoices);

  const currentFinance =
    calculateFinanceResults({
      cashIncome: currentRevenue,
      operatingExpenses:
        currentExpenses,
  ...
  const currentProfit = toNumber(
    currentFinance.operatingProfit,
  );

  const previousProfit = toNumber(
    previousFinance.operatingProfit,
  );

  const currentCashResult = toNumber(
    currentFinance.cashResult,
  );

  const previousCashResult = toNumber(
    previousFinance.cashResult,
  );

  const currentServicesList =
    services.filter(
      (item) =>
        item.createdAt >= currentStart,
    );

  const previousServicesList =
    services.filter(
      (item) =>
        item.createdAt < currentStart,
    );
  ...
  const allBusinessServices =
    await db.serviceRecord.findMany({
      where: {
        businessId,
      },
      select: {
  ...
  const serviceStatuses =
    Object.fromEntries(
      ACTIVE_SERVICE_STATUSES.map(
        (status) => [
          status,
          allBusinessServices.filter(
            (service) =>
  ...
  const mechanics =
    mechanicMemberships.map(
      ({ user }) => {
        ...
        const completed =
          assigned.filter((service) =>
            COMPLETED_SERVICE_STATUSES.includes(
              service.status,
            ),
          );
        ...
        const active =
          assigned.filter((service) =>
            ACTIVE_SERVICE_STATUSES.includes(
              service.status,
            ),
          );
  ...
  return {
    ...
    currentPurchases,
    currentCogs:
      toNumber(currentCogs),
    currentNetRevenue:
      toNumber(currentNetRevenue),
    currentProfit,
    currentCashResult,
    ...
    cashResultChange: percentageChange(
      currentCashResult,
      previousCashResult,
    ),
    currentServices:
    ...
    completedServices:
      currentServicesList.filter(
        (item) =>
          COMPLETED_SERVICE_STATUSES.includes(
            item.status,
          ),
      ).length,
    serviceChange: percentageChange(
      currentServicesList.length,
      previousServicesList.length,
    ),
    serviceStatuses,
    activeServiceTotal:
      Object.values(
    ...
    waitingForParts:
      serviceStatuses.WAITING_FOR_PARTS ||
      0,
    readyForPickup:
      serviceStatuses.READY_FOR_PICKUP ||
      0,
    inventoryValue: parts.reduce(
      (sum, part) =>
        sum +
        Number(part.stock || 0) *
      ...
      <div className="space-y-8">
        <div>
          ...
          <p className="mt-2 text-slate-500">
            Pamje operative nga databaza për
            detyrat e rolit tënd.
          </p>
        </div>
