...
export default async function WorkspacePage() {
  ...
  return <DashboardLayout><div className="space-y-7">
    <div><p className="text-sm font-bold text-blue-600">Roli: {businessRole}</p><h1 className="mt-2 text-3xl font-bold tracking-tight text-slate-950">{title}</h1><p className="mt-2 text-slate-500">{description}</p></div>
    <div className="grid gap-4 md:grid-cols-3">{stats.map(([label,value,Icon])=><div key={label} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><Icon className="text-blue-600" size={22}/><p className="mt-4 text-sm text-slate-500">{label}</p><p className="mt-1 text-3xl font-bold text-slate-950">{value}</p></div>)}</div>
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{(ACTIONS[businessRole]||[]).map(([label,href,Icon])=><Link key={label} href={href} className="group rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:border-blue-300"><Icon className="text-blue-600" size={22}/><div className="mt-4 flex items-center justify-between"><strong>{label}</strong><ArrowRight className="transition group-hover:translate-x-1" size={17}/></div></Link>)}</div>
    {businessRole !== "MECHANIC" ? <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><h2 className="text-lg font-bold">Aktiviteti i fundit i ekipit</h2><div className="mt-4 divide-y divide-slate-100">{recentActivity.length?recentActivity.map(item=><div key={item.id} className="py-3"><p className="text-sm font-semibold text-slate-900">{item.title}</p><p className="mt-1 text-xs text-slate-500">{item.user?.name||"Sistemi"} · {new Date(item.createdAt).toLocaleString("sq-AL")}</p></div>):<p className="py-4 text-sm text-slate-500">Nuk ka aktivitet.</p>}</div></section>:null}
  </div></DashboardLayout>;
}
