...
import { deletePart } from "@/actions/part-actions";

export default function DeletePartModal({ part, onClose }) {
  ...
  async function handleDelete() {
    try {
      ...
      if (!result?.success) {
        setError(result?.message || "Pjesa nuk mund të fshihet.");
        return;
      }
      ...
      <div className="w-full max-w-md rounded-[2rem] border border-slate-200 bg-white p-6 shadow-2xl">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xl font-bold text-slate-950">Fshi pjesën</h2>
        ...
        <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4">
          ...
          <p className="mt-2 font-bold text-slate-950">{part.name}</p>

          <p className="mt-1 text-sm text-slate-500">
            Kodi: {part.code || "Pa kod"} · Stoku: {part.stock}
          </p>
        </div>
