...
export default function ServiceWorkflowPanel({ service, staff }) {
    ...
    <div className="space-y-6">
      <form action={save} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-5 grid gap-5 md:grid-cols-2">
          <div><label className="mb-2 block text-sm font-semibold text-slate-700">Mekaniku përgjegjës</label><select name="assignedUserId" defaultValue={service.assignedUserId || ""} className="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm"><option value="">Pa caktuar</option>{staff.map(x=><option key={x.id} value={x.id}>{x.name} — {x.role}</option>)}</select></div>
          <div className="flex items-end gap-5 pb-3"><label className="flex items-center gap-2 text-sm font-medium text-slate-700"><input type="checkbox" name="customerApprovalRequired" defaultChecked={service.customerApprovalRequired}/> Kërko miratim klienti</label><label className="flex items-center gap-2 text-sm font-medium text-slate-700"><input type="checkbox" name="customerApproved" defaultChecked={Boolean(service.customerApprovedAt)}/> Miratuar</label></div>
          ...
          <div className="md:col-span-2"><label className="mb-2 block text-sm font-semibold text-slate-700">Shënime të brendshme</label><textarea name="internalNotes" defaultValue={service.internalNotes || ""} rows={4} className="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm" placeholder="Shënime vetëm për ekipin..."/></div>
        </div>
