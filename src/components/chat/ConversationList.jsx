...
export default function ConversationList({ conversations, basePath, emptyText }) {
  ...
  return <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm divide-y divide-slate-100">
    {conversations.map((conversation) => {
      ...
      return <Link key={conversation.id} href={`${basePath}/${conversation.id}`} className="group flex gap-4 p-5 transition hover:bg-slate-50/80">
        <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-blue-50 text-blue-600 transition group-hover:bg-blue-100"><MessageCircle size={20}/></div>
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-2 flex items-center justify-between gap-3"><p className="truncate text-sm text-slate-600">{lastMessage?.body || conversation.subject || "Bisedë e re"}</p>{conversation.unread ? <span className="h-2.5 w-2.5 shrink-0 rounded-full bg-blue-600 ring-4 ring-blue-50" title="Mesazh i palexuar"/> : null}</div>
        </div>
      </Link>;
    })}
  </div>;
