...
export default function DeleteCustomerModal({ customer, onClose }) {
  ...
  async function handleDelete() {
    try {
      ...
      if (!result?.success) {
        setError(result?.message || "Klienti nuk mund të fshihet.");
        return;
      }
      ...
      console.error(error);
      setError("Ndodhi një gabim gjatë fshirjes së klientit.");
    } finally {
      setIsDeleting(false);
    }
  }
      ...
      <div className="w-full max-w-md rounded-[2rem] border border-slate-200 bg-white p-6 shadow-2xl">
        ...
        <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4">
          <p className="text-sm text-slate-500">
            Je i sigurt që dëshiron të fshish klientin:
          </p>

          <p className="mt-2 font-bold text-slate-950">{customer.name}</p>
        ...
        <div className="mt-6 flex justify-end gap-3">
          ...
          <button
            ...
            disabled={isDeleting}
            className="flex items-center justify-center gap-2 rounded-full bg-red-600 px-5 py-3 text-sm font-bold text-white transition hover:bg-red-700 disabled:cursor-not-allowed disabled:opacity-60"
          >
            {isDeleting ? (
              <>
