...
export default function DeleteVehicleModal({ vehicle, onClose }) {
  ...
  async function handleDelete() {
    try {
      ...
      if (!result?.success) {
        setError(result?.message || "Automjeti nuk mund të fshihet.");
        return;
      }
      ...
      console.error(error);
      setError("Ndodhi një gabim gjatë fshirjes së automjetit.");
    } finally {
      setIsDeleting(false);
    }
  }
      ...
      <div className="w-full max-w-md rounded-[2rem] border border-slate-200 bg-white p-6 shadow-2xl">
        ...
        <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4">
          <p className="text-sm text-slate-500">
            Je i sigurt që dëshiron të fshish automjetin:
          </p>
